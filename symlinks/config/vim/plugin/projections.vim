if exists("g:loaded_projections") || &cp | finish | endif

let g:loaded_projections = 1

let s:vim_readme_template = [
			\ '# Name',
			\ '',
			\ '## Requirements',
			\ '',
...
let s:vim_plugin_config = {
			\ 'doc/*.txt': {
			\   'type': 'help',
			\   'template': s:vim_doc_template,
			\ },
			\ 'autoload/*.vim': {
			\   'type': 'autoload',
			...
			\ },
			\ 'README.md': {
			\   'template': s:vim_readme_template,
			\ },
			\ }


let s:lua_vim_plugin_config = {
			\ 'doc/*.txt': {
			\   'type': 'help',
			\   'template': s:vim_doc_template,
			\ },
...
function! s:c_project_config(source_extension, header_extension) abort
	return {
				\ '*': {
				\   'start': 'make run',
				\   'dispatch': 'make test',
				\ },
				\ 'src/*.' . a:source_extension: {
				\   'type': 'source',
				\   'alternate': [
				\     'src/{}.' . a:header_extension,
				\     'test/{}.' . a:source_extension,
				\     'include/{project|basename}/{}.' . a:header_extension,
				\   ],
				\ },
				\ 'test/*.' . a:source_extension: {
				\   'type': 'test',
...
let s:clojure_config = {
			...
			\ 'src/clj/*.cljs': {
			\   'type': 'source',
			\   'alternate': ['test/clj/{}_test.cljs'],
			\ },
			\ 'src/clj/*.cljc': {
			\   'type': 'source',
			\   'alternate': ['test/clj/{}_test.cljc'],
			\ },
			\ 'test/clj/*_test.clj': {
			\   'type': 'test',
			...
			\ 'test/clj/*_test.cljs': {
			\   'type': 'test',
			\   'alternate': ['src/clj/{}.cljs'],
			\ },
			\ 'test/clj/*_test.cljc': {
			\   'type': 'test',
			\   'alternate': ['src/clj/{}.cljc'],
			\ },
			\ }
...
let g:projectionist_heuristics = {
			\ 'pubspec.yaml': s:flutter_config,
			\ 'requirements.txt|pyproject.toml': s:python_config,
			\ '*.sln': s:dotnet_solution_config,
			\ '*.csproj': s:csharp_project_config,
			\ 'plugin/|autoload/': s:vim_plugin_config,
			\ 'src/*.cpp|test/*.cpp': s:c_project_config('cpp', 'hpp'),
			\ 'src/*.cc|test/*.cc': s:c_project_config('cc', 'hh'),
			\ 'src/*.c|test/*.c': s:c_project_config('c', 'h'),
			\ 'lua/': s:lua_vim_plugin_config,
			\ 'build.gradle|pom.xml': s:java_project_config,
			\ 'build.gradle|build.gradle.kts': s:kotlin_project_config,
			\ 'build.gradle|build.gradle.kts|settings.gradle': s:gradle_project_config,
			\ 'pom.xml': s:maven_project_config,
			\ 'mint.json': s:mint_config,
			\ 'shard.yml': s:crystal_config,
			\ 'Cargo.toml': s:rust_config,
			\ '.git&!Cargo.toml&*.rs': s:rust_freestanding_config,
			\ 'go.mod': s:golang_config,
			\ 'build.zig': s:zig_config,
			\ 'project.clj': s:lein_config,
